	...
	methods: {
		...
		async updatePages(notion, pages, limiter) {
			...
			for (let page of pages) {
				...
				const isTimeIncluded = due.includes("T")
				const nextDueAPI = page.properties[config.nextDueAPI.name].formula.string;
				const date = DateTime.fromISO(due, {setZone: true});
			
				const offsetNum = date.offset / 60;
				const offset = offsetNum.toString()

				let startDate = JSON.parse(nextDueAPI)['start'];
				let endDate = JSON.parse(nextDueAPI)['end'];

				console.log("isTimeIncluded: ", isTimeIncluded)
				let timeZoneString = ""
				if(isTimeIncluded) {
					timeZoneString = due.slice(-6);
				...
				endDate = (endDate == startDate) ? null : endDate + timeZoneString
				startDate = startDate + timeZoneString
				
				console.log(`Current Due value: ${due}`);
				console.log(`Current Due Start value: ${startDate}`);
				console.log(`Current Due End value: ${endDate}`);
